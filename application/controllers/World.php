...
class World extends CI_controller {
	...
	public function continent_countries($continent_name,$pnum=null){
		...
		$continent_name = str_replace('-',' ',$continent_name);
		$this->load->model('world_model');
		//var_dump($pnum);
		if(isset($pnum)){
	...
	public function country_edit($country_name){
		...
		if ($mode == 'confirm') {
			try{	
				...
				$this->session->set_userdata('country_data_edit',$this->input->post());
				
				$data = array();
				$this->custom_smarty->assign('data',$this->input->post());
				$this->custom_smarty->assign('body','application/views/pages/world_country_edit_confirm.tpl');
			...
			$this->custom_smarty->assign('body','application/views/pages/world_country_edit_complete.tpl');
			//update form
			$country_result = $this->world_model->update_country($this->session->userdata('country_data_edit'));
